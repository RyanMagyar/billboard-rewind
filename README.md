...
- Spotify OAuth uses a per-login random `state` value stored in the server-side session and validated on callback.
- The client cookie contains only a signed session id. Spotify tokens are not stored in browser cookies.
- Server-side session data is stored in Postgres using `express-session` and `connect-pg-simple`.
- Spotify access and refresh tokens are encrypted at rest with AES-256-GCM using `TOKEN_ENCRYPTION_KEY`.
- The session cookie is HTTP-only, uses `SameSite=Lax`, and is marked secure in production.
- Logout destroys the server-side session and clears the local session cookie.
- Auth, artist search, and playlist creation endpoints use rate limits to reduce abuse and accidental excessive Spotify/API traffic.